...
unsigned long new_yaw_time;
float new_yaw_time_interval = 50000;
float new_yaw_increment = 0;
int niclaOffset = 11;
...
float z_estimator = 0;
float new_yaw = 0;
//float new_yaw_time = 0;
//unsigned long new_yaw_time;
//float new_yaw_time_interval = 50000;
//float new_yaw_increment = 0;
//unsigned long new_yaw_increment;
...
void loop() {
  ...
  if (cmd.params[0] == 2) {
    ...
    if (nicla_flag != 0) 
    {// checks if new yaw occurs
        float _yaw = senses[5];  
        float _height = senses[1];  
        float tracking_x = (float)senses[niclaOffset + 1];
        float tracking_y = (float)senses[niclaOffset + 2];
        float tracking_w = (float)senses[niclaOffset + 3];
        float tracking_h = (float)senses[niclaOffset + 4];
       // Serial.print(tracking_x);
        ...
        float w_cal = tracking_w / terms.n_max_x;
        float h_cal = tracking_h / terms.n_max_y;
       // float new_yaw_time = 0;
       // float new_yaw_time_interval = 0;


        if ( abs(x_cal - 0.5) < .16 && terms.y_strength != 0) 
        { // makes sure yaw is in center before making height adjustments
            z_estimator =  ( _height + terms.y_strength * (y_cal - terms.y_thresh)); // height control doesnt work well when not 0 bouyant
            forward_force = terms.fx_togoal;
            Serial.print(tracking_x);
        } else {
          z_estimator =  ( _height + terms.y_strength * (y_cal - terms.y_thresh));
          behave.params[4] = new_yaw;
        }
    } 
    else{
      if (micros() - new_yaw_time > new_yaw_time_interval) {
        new_yaw += new_yaw_increment;
        if (new_yaw >= 3.14){
          new_yaw = -2*3.14;
        }
        new_yaw_time = micros();
    ...
    behave.params[1] = cmd.params[1] + (forward_force*0.7) ; // fx ('meters'/second)
  ...
  { // direct control with joystick if 'flag' is not 2
